#include "fichier.h"
int saisir() { int x;
    printf("saisir le nombre de patients ") ;  scanf("%d",&x) ;
    return x;}
int saisir1()
{ int x;
    printf("\n saisir le nombre des dossier medicaux ") ;  scanf("%d",&x) ;
    return x;}
MED * AllocationMedicaments(int n)
{MED* medicament ;
    medicament=(MED*) malloc (n * sizeof (MED));
    if(!medicament) exit(-4);  return medicament ;}
...
{printf("\n saisir les visites %d ",j+1);
      printf("\n saisir la date du rendez-vous %d ",j+1); scanf("%d%d%d", &visites->date_RDV.jour,&visites->date_RDV.mois, &visites->date_RDV.annee);
...
{for(int l=0;l<n;l++)
    {printf("\n saisir l'identifiant de patient "); scanf("%d",&(*(d+l))->idPatient);
        printf("\n saisir l'etat sanitaire ");  scanf("%s",&(*(d+l))->etat_sanitaire); }}
...
{printf("\n saisir le nom et le prenom ");  scanf("%s", &p->nom_prenom);
        printf("\n saisir le sexe du patient ");  scanf("%s", &p->sexe);
        printf("\n saisir la profession "); scanf("%s", &p->profession);
        printf("\n saisir la date de naissance "); scanf("%d%d%d", &p->date_naiss.jour,&p->date_naiss.mois,&p->date_naiss.annee);
        printf("\n saisir son age ");  scanf("%d", &p->age);
        printf("\n saisir son numero de telephone ");   scanf("%d", &p->num_telephone);
        printf("\n saisir le nombre de visites "); scanf("%d", &p->nbr_visites);
...
{printf("\n les infos du patient %d ", i+1);
    ...
    for(int j=0; j< p.nbr_visites;j++)  AfficherVisite(*(p.visites+j));}
...
{ int m,a; int R=0;
    printf("saisir le mois et l'annee desires :"); scanf("%d%d",&m,&a); 
    for (int i=1; i<32;i++)
    {DATE dat; dat.jour=i; dat.mois=m; dat.annee=a;  R+=RecetteJournaliere(p,dat,n); }
    return R;}
float RecetteJournaliere(PATIENT*p,DATE date,int n)
{ int nb;  nb=NbrePatient(p,date,n);
    return nb*70;}
...
int MaxNbreVisites(PATIENT*p,int n)
 {int max; max=1;
    for(int i=0;i<n;i++)   {if (p[i].nbr_visites > max)  max=p[i].nbr_visites;} return max;}
